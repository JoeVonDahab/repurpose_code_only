...
import esm
# Note: load_model_and_alphabet_hub is deprecated, but we keep it for compatibility with the original code.
# The modern way is from esm.pretrained import esm2_t33_650M_UR50D
...
class ESMCalculator:
    ...
    def model(self):
        ...
        # FIX 1: Only initialize distributed backend if a GPU is available.
        if torch.cuda.is_available() and not torch.distributed.is_initialized():
            url = "tcp://localhost:23456"
            torch.distributed.init_process_group(backend="nccl", init_method=url, world_size=1, rank=0)

        # FIX 2: Use a cache directory in your home folder instead of a hardcoded path.
        torch_hub_cache = os.path.join(os.path.expanduser("~"), ".cache", "torch_hub_esm")
        ...
        # This FSDP wrapping is only for GPU execution.
        if torch.cuda.is_available():
            fsdp_params = dict(
