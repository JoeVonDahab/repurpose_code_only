...
def main():
    parser = argparse.ArgumentParser(description="Rank docking results and copy top ligands")
    parser.add_argument("--dlg-dir", type=Path, default="docking_results_compiled",
                       help="Directory containing DLG files (default: docking_results_compiled)")
    parser.add_argument("--pdbqt-dir", type=Path, default="docked_converted", 
                       help="Directory containing PDBQT files (default: docked_converted)")
    parser.add_argument("--output-dir", type=Path, default="top_100_ligands",
                       help="Output directory for top ligands (default: top_100_ligands)")
    parser.add_argument("--csv-output", type=Path, default="docking_results_ranked.csv",
                       help="Output CSV file (default: docking_results_ranked.csv)")
    parser.add_argument("--smiles-file", type=Path, default="smiles.smi",
                       help="SMILES input file (default: smiles.smi)")
    parser.add_argument("--top-n", type=int, default=100,
                       help="Number of top ligands to copy (default: 100)")
    
    args = parser.parse_args()
    
    INPUT_DLG_DIR = args.dlg_dir
    INPUT_PDBQT_DIR = args.pdbqt_dir
    TOP_OUT_DIR = args.output_dir
    CSV_OUT = args.csv_output
    SMILES_INPUT_FILE = args.smiles_file
    TOP_N = args.top_n
